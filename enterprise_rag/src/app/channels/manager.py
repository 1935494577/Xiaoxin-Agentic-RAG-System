"""ChannelManager — consumes inbound messages and dispatches them to the Jnao agent via Gateway."""
...
BOUND_IDENTITY_REQUIRED_MESSAGE = "Connect this channel from Jnao Settings, complete the in-channel connect step, then send your message again."
BOUND_IDENTITY_UNAVAILABLE_MESSAGE = "Channel connection verification is temporarily unavailable. Please try again later or contact the Jnao operator."
...
def _channel_storage_user_id(msg: InboundMessage) -> str | None:
    """Resolve the canonical Jnao user id for a channel-triggered message.
    ...
    matches where channel files are staged. Prefer the bound Jnao owner,
...
class ChannelManager:
    """Core dispatcher that bridges IM channels to the Jnao agent.
    ...
    def _resolve_run_params(self, msg: InboundMessage, thread_id: str) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        # ``user_id`` drives Jnao-owned memory, files, and thread buckets.
        # For browser-connected IM channels, prefer the Jnao account that
    ...
    async def _get_bound_identity_rejection(self, msg: InboundMessage) -> _BoundIdentityRejection | None:
        ...
        # the binding by provider identity before creating Jnao threads or
    ...
    async def _handle_chat(
        ...
        *,
        ...
        # Look up the existing Jnao thread, creating one if this is the
