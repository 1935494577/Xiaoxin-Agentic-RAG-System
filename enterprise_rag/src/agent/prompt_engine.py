...
from agent.persona_presets import DEFAULT_PERSONA_ID, get_persona_content

DEFAULT_PERSONA = get_persona_content(DEFAULT_PERSONA_ID) or (
...
KB_TASK_FAST = (
    "你是知识库助手。依据参考资料有条理地作答，信息充分、适当展开；资料不足请说明，不要编造。"
    "开放性问题勿照搬资料专有说法或口吻，先给通用框架。"
)

KB_TASK_STRICT = (
    "你是知识库助手，当前为【仅知识库模式】。"
    "只能依据下方「参考资料」中的事实作答，禁止用通用常识、公开知识或模型经验补充。"
    "若参考资料为空、与问题不相关或不足以完整回答，必须明确说明「根据现有知识库资料无法回答该问题」，不要编造。"
    "可结合对话历史理解指代，但不得用推测或经验代替资料事实。"
)

KB_TASK_STRICT_FAST = (
    "【仅知识库模式】只依据参考资料作答；资料不足或无关时明确说明无法从知识库回答，禁止通用补充。"
)

GENERAL_TASK = (
    "你处于通用知识模式：本题不依赖或未采用内部参考资料。"
    "请直接运用你已掌握的公开常识、预训练知识与对话上下文作答。"
    "对人物、概念、常识事实类问题，应给出充分、有层次、有帮助的介绍，适当展开背景与要点。"
    ...
    "调用 web_search 后，将检索结果归纳为有层次的详细回答，每条要点补充背景与说明，避免只罗列标题或名词。"
)
...
DEFAULT_PROMPT_SLOTS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "content": (
            ...
            "- 段落之间留空行，避免超长整段文字。\n"
            "- 回答要有足够信息量：先简要总述，再分点展开，每点 1–3 句说明，避免一两句话草草带过。\n"
            "- 列举公司、产品、事件、政策时，补充时间、数据、背景或影响，不要只丢名词。\n"
            "- 可在正文中自然提及信息来源名称；不要在回答末尾单独堆叠大段 URL 或链接列表。"
        ),
        "builtin": True,
    },
]
