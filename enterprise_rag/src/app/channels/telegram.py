...
class TelegramChannel(Channel):
    ...
    async def _bind_connection_from_start_token(self, update, state_token: str) -> bool:
        ...
        logger.info("[Telegram] bound chat=%s user=%s to Jnao user=%s connection=%s", chat_id, user_id, owner_user_id, connection["id"])
        await update.message.reply_text("Telegram connected to Jnao.")
        return True
    ...
    async def _cmd_start(self, update, context) -> None:
        ...
        if not self._check_user(update.effective_user.id):
            return
        await update.message.reply_text("Welcome to Jnao! Send me a message to start a conversation.\nType /help for available commands.")
    ...
    async def _on_text(self, update, context) -> None:
        ...
        # topic_id determines which Jnao thread the message maps to.
