...
class WeComChannel(Channel):
    ...
    async def _send_ws_upload_command(self, req_id: str, body: dict[str, Any], cmd: str) -> dict[str, Any]:
        ...
        if not callable(send_reply):
            raise RuntimeError("Installed wecom-aibot-python-sdk does not expose the WebSocket media upload API expected by Jnao. Use wecom-aibot-python-sdk==0.1.6 or update the adapter.")
    ...
    async def _bind_connection_from_connect_code(self, *, frame: dict[str, Any], user_id: str, code: str) -> bool:
        ...
        await self._send_connection_reply(frame, "WeCom connected to Jnao.")
        return True
